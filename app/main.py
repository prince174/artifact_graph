...
def graph(q: str = ""):
    ...
    if q:
        seeds = {n["id"] for n in result_nodes if q.lower() in n["label"].lower() and n["kind"] == "repository"}
        connected = seeds | {e["source"] for e in result_edges if e["target"] in seeds} | {e["target"] for e in result_edges if e["source"] in seeds}
        for _ in range(2):
            connected |= {e["target"] for e in result_edges if e["source"] in connected}
        result_nodes = [n for n in result_nodes if n["id"] in connected]
        ids = {n["id"] for n in result_nodes}; result_edges = [e for e in result_edges if e["source"] in ids and e["target"] in ids]
